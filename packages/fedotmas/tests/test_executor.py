...
async def test_terminate_is_any_of_in_list_order():
    system = System([as_node(emit("tick"), name="t", reads="tick")])
    run = await ReactiveExecutor().run(
        system,
        Store(),
        seed=[Fact(tag="tick")],
        terminate=[Goal("tick"), Budget(9)],
    )
    assert len(run.steps) == 1
    assert run.reason == "goal"


async def test_a_custom_terminate_names_the_reason_with_its_class():
    class Deadline:
    ...
    system = System([as_node(emit("tick"), name="t", reads="tick")])
    run = await ReactiveExecutor().run(
        system, Store(), seed=[Fact(tag="tick")], terminate=[Deadline()]
    )
