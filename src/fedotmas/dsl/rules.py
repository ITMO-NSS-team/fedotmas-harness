"""The rule surface: a thin blackboard helper, peer to the combinators.

A Rule pairs an author-written condition with a step. Unlike a combinator there is no
topology to derive a trigger from, so the author writes `when` directly; the helper only
owns the Result/Fact boilerplate. This is the blackboard superset that the edge-shaped
combinators cannot express (opportunistic, non-linear activation).
"""
...
from fedotmas.dsl.combinators import StepFn
